...
Requires nutshell-server to be running for real-time updates.
...
class NewSessionModal(ModalScreen):
    ...
    def compose(self) -> ComposeResult:
        with Vertical(id="modal-box"):
            ...
            yield Input(value="agent", id="modal-entity")
    ...
    def _submit(self) -> None:
        ...
        entity = self.query_one("#modal-entity", Input).value.strip() or "agent"
...
class NutshellTUI(App):
    ...
    def _render(self, event: dict) -> None:
        """Render a display event to the chat log."""
        log = self.query_one("#chat-log", RichLog)
...
def main() -> None:
    ...
    load_dotenv()
    parser = argparse.ArgumentParser(description="Nutshell TUI — terminal monitoring client")
