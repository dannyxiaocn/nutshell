...
- `task_cards.py`: task card system — per-task `.md` files in `core/tasks/` with YAML frontmatter (`interval`, `starts_at`, `ends_at`, `status`, `last_run_at`), legacy `tasks.md` / `default_task` migration, and single-card load/save helpers used by the UI.
...
- Persistent sessions use the recurring `core/tasks/heartbeat.md` card as their canonical heartbeat task. Older `default_task` values are migrated into that card.
