...
## Step 7 — Update agent memory (cross-session persistence)

Update the agent's memory files so future sessions inherit the knowledge:

```bash
# In playground/butterfly/
# 1. Update agent memory.md: bump version in "Recent Changes" section

# 2. Update work_state layer
COMMIT_ID=$(git rev-parse --short HEAD~1)   # feature commit (before track commit)
cat > agenthub/butterfly_dev/memory/work_state.md << 'EOF'
# Work State
...
git add agenthub/butterfly_dev/memory/ agenthub/butterfly_dev/memory.md
git commit -m "butterfly_dev: update agent memory after vX.Y.Z"
```
