...
class GitCoordinator:
    ...
    def register(self, repo_path: Path, session_id: str | None = None) -> Role:
        ...
        Args:
            ...
            session_id: Session ID (defaults to BUTTERFLY_SESSION_ID env var).

        Returns:
            "master" or "sub".
        """
        session_id = session_id or os.environ.get("BUTTERFLY_SESSION_ID", "")
        if not session_id:
            return "sub"
    ...
    def release(self, session_id: str | None = None) -> list[str]:
        ...
        Args:
            session_id: Session to release (defaults to BUTTERFLY_SESSION_ID).
