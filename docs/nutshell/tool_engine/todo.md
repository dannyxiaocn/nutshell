...
- [ ] **CLI app tool auto-auth**: 当 agent 调用外部 CLI 工具（gh, gcloud, aws, npm 等）时，runtime 应自动注入认证信息（token, API key, session cookie），agent 只传业务参数。实现路径：executor 构造时读取 `~/.nutshell/credentials/` 或 session-level credential store，注入到 env vars 或 CLI flags
...
- [ ] **Session env auto-injection**: 所有 toolhub executor 自动继承 session 环境变量（`NUTSHELL_SESSION_ID`, `NUTSHELL_ENTITY`, venv `PATH`），不需要每个 executor 单独处理
