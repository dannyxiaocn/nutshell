...
- [ ] 加 layering linter：`tests/test_ui_layering.py`，用 `ast` 静态扫描 `ui/**/*.py`，禁止 import `butterfly.runtime.ipc` / `butterfly.session_engine.session_status` / `butterfly.session_engine.session_params` / `butterfly.runtime.bridge`
- [ ] 前端：把 `_sse_format`、任务 interval 校验这类从 Python 搬到 TS，让 app.py 真的只剩 `route → service(args) → json`

## Future

- [ ] `butterfly/service/` → `butterfly/commands/`，每个文件 = 一个 CLI 命令，CLI 和 Web 都降级成纯分发器。需先完成上面所有 follow-up，独立 PR
